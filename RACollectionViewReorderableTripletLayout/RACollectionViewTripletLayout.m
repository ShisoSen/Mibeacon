...
//    }
    NSInteger cellsCount = [self.collectionView numberOfItemsInSection:self.collectionView.numberOfSections-1];
    if (cellsCount%3!=0) {
        contentSize.height -= [_smallCellSizeArray[self.collectionView.numberOfSections - 1] CGSizeValue].height + _itemSpacing;
    }
    return contentSize;
...
// bug fix - crash in attributes out of bounds @sicong
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSInteger cellsCount = [self.collectionView numberOfItemsInSection:indexPath.section];
    if (cellsCount%3==1) {
        if (indexPath.item==cellsCount-1) {
            attribute.frame = CGRectMake(_insets.left, lineOriginY, _collectionViewSize.width-_insets.left-_insets.right, _smallCellSize.height);
        }
    }else if (cellsCount%3==2){
        if (indexPath.item==cellsCount-2) {
            attribute.frame = CGRectMake(_insets.left, lineOriginY, _smallCellSize.width, _smallCellSize.height);
        }else if(indexPath.item==cellsCount-1){
